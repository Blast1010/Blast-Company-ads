...
function FloatingElements() {
  ...
  const smallBoxRef = useRef<THREE.Mesh>(null);
  ...
  useFrame((state) => {
    ...
    if (smallBoxRef.current) {
      smallBoxRef.current.rotation.x = time * 0.4;
      smallBoxRef.current.rotation.y = time * 0.6;
    }
  });
  ...
  return (
    <group>
      ...
      <Float speed={3} rotationIntensity={2} floatIntensity={3}>
        <Box ref={smallBoxRef} args={[0.4, 0.4, 0.4]} position={[-2, -2, 2]}>
          <meshStandardMaterial 
            color="#ef4444" 
            metalness={0.8} 
            roughness={0.2}
